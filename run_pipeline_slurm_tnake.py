...
def write_slurm(field_name):
    with open("/project/apdw/Software/aper_sf2/slurm_out/slurm_"+field_name+"_"+date_string+".sh", "w") as f:
        ...
        f.write("#SBATCH --cpus-per-task=30             # Cores per task (>1 if multithread tasks)\n")
        ...
        f.write('snakemake --cores 29 -s Tnakefile_slurm --use-singularity --singularity-args "--bind /project/apdw/" --resources mem_mb=225000 --config FIELD='+field_name)
