...
def main(loc, taskid, beam=[40], cubes=[1, 2, 3], nospline=False, mosaic=False):
    ...
    for b in beam:
        ...
        if len(results) > 0:
            ...
            # Probably broken for non mosaic
            # If not mosaic, plot all integrated spectra on a single plot.
            if not mosaic:
                for c in cubes:
            ...
            for c in cubes:
                ...
                if os.path.isfile(loc + cube_name + '_sofiaFS_cat.txt'):
                    ...
                    if (len(cat) > max_cat_len) and (not mosaic):
                        ...
                        print("\t{} candidates found in the mosaic".format(len(cat)))
                    ...
                    for s in range(len(cat)):
                        ...
                        # print(f"Do {s} subcube: {toc1 - tic1:0.4f} seconds")

                        panel = previous + s
                        # If working with mosaic cube, only plot 20 spectra at a time...
                        if mosaic:
                        ...
                        # Plot spectra
                        # tic1 = testtime.perf_counter()
                        ax_spec[panel, 0].plot([optical_velocity[-1].value, optical_velocity[0].value], [0, 0], '--',
                                               color='gray')
                        ax_spec[panel, 0].plot(optical_velocity, spectrum, c=colors[c-1])
                        ax_spec[panel, 0].plot([maskmin, maskmin], [np.nanmin(spectrum), np.nanmax(spectrum)], ':',
                                               color='gray')
                        ax_spec[panel, 0].plot([maskmax, maskmax], [np.nanmin(spectrum), np.nanmax(spectrum)], ':',
                                               color='gray')
                        ax_spec[panel, 0].set_title("Beam {:02}, Cube {}, Source {}".format(b, c, cat['col2'][s]),
                                                    fontsize=20)
                        ax_spec[panel, 0].set_xlim(optical_velocity[-1].value, optical_velocity[0].value)
                        if (np.nanmax(spectrum) > 2.) | (np.nanmin(spectrum) < -2.):
                            ax_spec[panel, 0].set_ylim(np.nanmax(spectrum[cat['col10'][s]:cat['col11'][s]]) * -2,
                                                       np.nanmax(spectrum[cat['col10'][s]:cat['col11'][s]]) * 2)
                        ax_spec[panel, 0].set_ylabel("Integrated Flux")
                        # toc1 = testtime.perf_counter ()
                        # print(f"Do {s} plotting: {toc1 - tic1:0.4f} seconds")

                        # Add x-axis label at the end of making spec plots
                        if mosaic:
