...
def main(taskid, beam=[40], nospline=False, mosaic=False):
    ...
    for b in beam:
        ...
        # loc = taskid + '/B0' + str(b).zfill(2) + '/'              DELETE THIS WHEN NECESSARY *******************************
        # if mosaic:                                              DELETE THIS WHEN NECESSARY *******************************
        loc = taskid + '/'
        source_per_beam = 0
        ...
        if len(results) > 0:
            ...
            wcs = WCS(header)
            if mosaic:
                fig_im, ax_im = plt.subplots(1, 1, figsize=(15, 12), subplot_kw={'projection': wcs})
            else:
            ...
            for c in cubes:
                ...
                if os.path.isfile(loc + cube_name + '_sofia_cat.txt'):
                    ...
                    cube_frequencies = chan2freq(np.array(range(hdu_spline[0].data.shape[0])), hdu=hdu_spline[0].header)
                    ...
                    for s in range(len(cat)):
                        ax_im[c-1].text(cat['col3'][s] + np.random.uniform(-40, 40),
                        ...
                        maskmin = chan2freq(cat['col10'][s],
                                            hdu=hdu_spline[0].header).to(u.km/u.s, equivalencies=optical_HI).value
