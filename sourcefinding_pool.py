...
def run(i):
    ...
    #commented out debug print (don't use unless you have a small data cutout)
...
parser.add_argument('-j', "--njobs", required=False,
                    ...
                    default=18)
#commented out because chunksize is determined by the shape of the data
#parser.add_argument('-s', "--chunksize", required=False,
#                    help="Chunk size to pass to Multiproccessing Pool",
#                    default=100)
...
for b in beams:
    ...
    for c in cubes:
        ...
        if (not overwrite) & os.path.isfile(splinefits):
            ...
            #changing to close original cube  here to free up data 
            orig.close()
            splinecube.close()
            ...
            #TNH: experimenting with using Pool instead to introduce a chunk size
            print("Starting Pool with njobs="+str(njobs)+"and chunksize="+str(chunksize))
            tic1 = testtime.perf_counter()
            with Pool(processes=njobs) as pool:
                #runs spline fitting andn saves fits as array
                fits_list = np.array(list(pool.imap(run, trange(ncases), chunksize=chunksize)))

            # exiting the 'with'-block has stopped the pool
            #print("The fits are complete, and now the data will be written to file.")

            toc1 = testtime.perf_counter()
            print(f"Do splinefit: {toc1 - tic1:0.4f} seconds")
